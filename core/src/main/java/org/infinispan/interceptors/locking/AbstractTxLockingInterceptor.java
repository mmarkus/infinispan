...
public class AbstractTxLockingInterceptor extends AbstractLockingInterceptor {
   ...
   public Object visitCommitCommand(TxInvocationContext ctx, CommitCommand command) throws Throwable {
      ...
      return super.visitCommitCommand(ctx, command);
         ...
         //if context is remote then locks are being acquired lazily
         if (ctx.isOriginLocal() || configuration.isSecondPhaseAsync()) lockManager.unlockAll(ctx);
      }
   }
   ...
   protected final Object invokeNextAndCommitIf1Pc(TxInvocationContext ctx, PrepareCommand command) throws Throwable {
      ...
      if (command.isOnePhaseCommit() && (ctx.isOriginLocal() || configuration.isSecondPhaseAsync())) {
         lockManager.unlockAll(ctx);
      }
      return result;
   }
   ...
   protected final void lockAndRegisterBackupLock(TxInvocationContext ctx, Object key) throws InterruptedException {
      if (cll.localNodeOwnsLock(key)) {
         lockKeyAndCheckOwnership(ctx, key);
      } else if (cll.localNodeIsOwner(key)) {
   ...
   protected final void lockKeyAndCheckOwnership(InvocationContext ctx, Object key) throws InterruptedException {
      ...
      if (!checkForPendingLocks) {
         ...
         long remaining = configuration.getLockAcquisitionTimeout();

         //first wait for all potential lock owners
         for (CacheTransaction ct : tx) {
            if (!ct.waitForTransactionsToFinishIfHasLock(key, remaining)) {
               throw new TimeoutException("Could not acquire lock on " + key + " on behalf of transaction " +
                                                txContext.getGlobalTransaction() + ". Lock is being held by " + lockManager.getOwner(key));
            }
            final long elapsed = System.currentTimeMillis() - start;
            remaining = Math.abs(remaining - elapsed);
         }
